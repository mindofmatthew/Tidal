...
import           Data.Maybe (fromJust, fromMaybe, catMaybes, mapMaybe, isJust)
...
streamReplace :: ID a => Stream -> a -> ControlPattern -> IO ()
streamReplace s k !pat
  = E.catch (do let x = queryArc pat (Arc 0 0)
                ...
                putMVar (sStateMV s) $
                  Map.insert ("_t_all") (VR cyc) $ Map.insert ("_t_" ++ toID k) (VR cyc) input
                -- update the pattern itself
                pMap <- seq x $ takeMVar $ sPMapMV s
                let playState = updatePS $ Map.lookup (toID k) pMap
                putMVar (sPMapMV s) $ Map.insert (toID k) playState pMap
                return ()
          )
...
streamMute :: IDs a => Stream -> a -> IO ()
streamMute s k = withPatIds s (toIDs k) (\x -> x {mute = True})

streamUnmute :: IDs a => Stream -> a -> IO ()
streamUnmute s k = withPatIds s (toIDs k) (\x -> x {mute = False})

streamSolo :: IDs a => Stream -> a -> IO ()
streamSolo s k = withPatIds s (toIDs k) (\x -> x {solo = True})

streamUnsolo :: IDs a => Stream -> a -> IO ()
streamUnsolo s k = withPatIds s (toIDs k) (\x -> x {solo = False})
  ...
  = do ms <- recvMessagesTimeout 2 sock
     ...
     where
        ...
        act (O.Message "/unsolo" ks)
          = streamUnsolo stream $ mapMaybe datumToID ks
        ...
        add k v = do sMap <- takeMVar (sStateMV stream)
                     ...
                     return ()
        datumToID :: O.Datum -> Maybe String
        datumToID (O.ASCII_String k) = Just $ O.ascii_to_string k
        datumToID (O.Int32 k) = Just $ show k
        datumToID _ = Nothing
